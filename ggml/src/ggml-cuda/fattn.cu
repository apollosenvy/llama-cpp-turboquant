...
static bool ggml_cuda_fattn_kv_type_supported(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q8_0:
        case GGML_TYPE_BF16:
        // TurboQuant KV cache types (fork): dim constraints checked at call site
...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor * dst) {
    ...
    if (!ggml_cuda_fattn_kv_type_supported(K->type) || !ggml_cuda_fattn_kv_type_supported(V->type)) {
        return BEST_FATTN_KERNEL_NONE;
    // TurboQuant KV: VEC kernels instantiated for D in {64, 128, 256} (mult of 64)
    ...
    const int ncols2_max = Q->ne[0] == 320 ? 32 : ((Q->ne[0] == 576 || Q->ne[0] == 640 || Q->ne[0] == 192) ? 16 : 8); // fork: 640 = MLA turbo pad
    int gqa_ratio_eff = 1;
