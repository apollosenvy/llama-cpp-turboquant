    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->op == GGML_OP_MUL_MAT_ID) {
            ...
            if (ggml_cuda_mul_mat_id_needs_sync(node, cc) || is_tq_w) {
                // the mul_mat_id fallback path synchronizes the stream, so we cannot use CUDA graph
                // (fork: TQ fused path also synchronizes)
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_SET_ROWS:
            {
                // fork: turbo KV types require head_dim divisible by their group size
