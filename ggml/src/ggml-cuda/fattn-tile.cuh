...
static void launch_fattn_tile_switch_ncols2(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // MLA turbo pads (GLM K=576, Instella K=544->640, DeepSeek-V4 MLA turbo 576->640).
    // Prefer GQA-opt when available; fall back to ncols=1 so load probes and
    // no-mask graphs still run. (Fork: 640 added to upstream's 576 handling.)
