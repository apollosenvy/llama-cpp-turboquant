...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // fork: +3 for turbo rotation matrices (turbo_rotation + turbo_rotation_inv + turbo_innerq_scale_inv)
    ...
    if (other) {
        ...
        // fork: turbo KV types perform WHT rotation at kernel level; graph-level
        // rotation on top would double-rotate. Turbo owns rotation for its types.
